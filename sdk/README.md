# Sandbox Host compatibility SDK

This package is derived from Vercel's Apache-2.0 licensed `@vercel/sandbox`
SDK and preserves its public programming model. It adds a configurable API
origin and local bearer-token environment variables:

```sh
SANDBOX_API_URL=https://your-sandbox-host.example/api
SANDBOX_TOKEN=...
SANDBOX_TEAM_ID=default
SANDBOX_PROJECT_ID=default
```

Applications can import `@sandbox-host/sdk`, or install this tarball under an
npm alias named `@vercel/sandbox` to preserve existing imports.

# Upstream Vercel Sandbox documentation
