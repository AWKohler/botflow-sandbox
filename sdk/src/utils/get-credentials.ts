...
export async function getCredentials(params?: unknown): Promise<Credentials> {
  ...
  const localToken = process.env.SANDBOX_TOKEN;
  const localTeamId = process.env.SANDBOX_TEAM_ID;
  const localProjectId = process.env.SANDBOX_PROJECT_ID;
  const localValues = [localToken, localTeamId, localProjectId].filter(Boolean);
  if (localValues.length > 0 && localValues.length < 3) {
    throw new Error(
      "SANDBOX_TOKEN, SANDBOX_TEAM_ID, and SANDBOX_PROJECT_ID must be set together",
    );
  }
  if (localToken && localTeamId && localProjectId) {
    return {
      token: localToken,
      teamId: localTeamId,
